...
public class GiftRepeatItemView extends LinearLayout {
    ...
    private ImageView user_header;  //用户头像
    private TextView user_name;     //用户名
    private TextView gift_name;     //礼物名称
    private ImageView gift_img;     //礼物图片
    private TextView gift_num;      //礼物送出数量

    //动画
    private Animation viewInAnim;   //用户动画
    private Animation imgViewInAnim;//礼物动画
    private Animation textScaleAnim;//数量动画
    ...
    public void showGift(GiftInfo giftInfo, String repeatId, TIMUserProfile profile) {
        ...
        if (getVisibility() == INVISIBLE) {
            ...
            if (TextUtils.isEmpty(avatarUrl)) {
                UtilImg.loadRound(R.drawable.default_avatar, user_header);
            } else {
                UtilImg.loadRound(avatarUrl, user_header);
            }
            ...
            UtilImg.load(giftInfo.giftResId, gift_img);
